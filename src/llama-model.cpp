        ...
        case LLM_ARCH_QWEN3OMNIMOE:
            {
                ...
                ml.get_key(LLM_KV_ATTENTION_LAYERNORM_RMS_EPS, hparams.f_norm_rms_eps);
                // For now, use the same type mapping as QWEN3MOE
                switch (hparams.n_layer) {
                    case 48: type = LLM_TYPE_30B_A3B; break;
                    case 94: type = LLM_TYPE_235B_A22B; break;
                    default: type = LLM_TYPE_UNKNOWN;
...
struct llm_build_qwen3omnimoe : public llm_graph_context {
    llm_build_qwen3omnimoe(const llama_model & model, const llm_graph_params & params) : llm_graph_context(params) {
        const int64_t n_embd_head = hparams.n_embd_head_v;
...
ggml_cgraph * llama_model::build_graph(const llm_graph_params & params) const {
    ...
    switch (arch) {
        ...
        case LLM_ARCH_QWEN3OMNIMOE:
            {
                llm = std::make_unique<llm_build_qwen3omnimoe>(*this, params);
            } break;
        case LLM_ARCH_PHI2:
            {
